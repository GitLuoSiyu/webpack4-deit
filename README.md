...
接下来就可以运行`npm start`，观察进阶配置后的结果，使用`npm run dev`启动，不用重复打包`vendor`。生产环境打包使用的是`npm run build`。

以上就是对`webpack4.x`配置的踩坑过程，本次webpack配置我也是参考github上的大佬们，一步步踩坑学到的。借鉴地址[`github`地址](https://github.com/wlx200510/webpack4.x-learn)学习过程中 也非常感谢各位大佬的耐心指教。
